...
Zalomení stránky lze vynutit nebo potlaèit pøíkazem:
...
%options]
Parametry se zadávají uvnitø hranatých závorek. Parametry a jejich smysl jsou
uvedeny v~\in{tabulce}[tab:page options].
...
\NC \type{makeup}        \NC zaèátek nové stránky bez plovoucích objektù\NC\MR
\NC \type{no}            \NC potlaèení stránkového zlomu \NC\MR
...
%a page number with:
Èíslování stránek je automatické, ale je mo¾né ruènì nastavit èíslo stránky
pøíkazem:
...
%\type{[+2]} or \type{[-2]}.
Nìkdy je lep¹í zadat èíslo strany relativnì, napøíklad
\type{[+2]} nebo \type{[-2]}.
...
Umístìní èísla stránek na stranì závisí na na¹ich preferencích a na typu
dokumentu (jednostranný nebo dvojstranný). Èíslování stránek se
nastavuje pøíkazem:

\shortsetup{setuppagenumbering}

%The preferences are placed within the brackets.
Parametry se umís»ují do hranatých závorek.

%Tables or figures may take up a lot of space. The placing of
%these text elements can be postponed till the next page break.
%This is done with:
Nìkdy mù¾e být výhodné odlo¾it nìjaký materiál a¾ na zaèátek dal¹í
strany, tedy po provedení nejbli¾¹ího stránkového zlomu.
To se provede pøíkazem:
...
aby co nejménì naru¹oval prùbì¾ný text.
