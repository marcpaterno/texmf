...
\type{\footnote}. La commande s'utilise de la manière suivante :
...
afin de pouvoir y faire référence. Les accolades contiennent le texte
que vous voulez afficher au bas de la page.

% The same footnote number can be called with its logical name.

Le numéro d'une note de bas de page peut être cité en utilisant son nom
logique.
...
La numérotation des notes de bas de page est automatique. La commande
\type{\setupfootnote} vous permet de configurer leur présentation.
...
Les notes de bas de page peuvent être placées au bas de la page mais,
également, à d'autres endroits comme, par exemple, à la fin d'un
chapitre. On peut réaliser cela avec la commande :
...
On peut également placer des notes dans un tableau. Dans ce cas, on
